...
#		@server.debug = true
	...
	def init_ready
		...
		# connection info
		@server.input_registers[1] = 0xffff	# 1-00 to 1-15
		@server.input_registers[2] = 0xffff	# 2-00 to 2-15
		@server.input_registers[3] = 0xffff	# 3-00 to 3-15
		@server.input_registers[4] = 0xffff	# 4-00 to 4-15
		1000.step 1189, 3 do |r|
			@server.input_registers[r] = (cap_hp + cap_fanstep3)
			@server.input_registers[r] = (cap_hp + cap_fanstep2) if(r%5 == 0)
			@server.input_registers[r] = (cap_hp + cap_fanstep5 + cap_swing) if(r%2 == 0)
		...
		1.upto 64 do |id|
			...
			cool_heat_master id, 1 if((id-1)%8 == 0)
			# set holding registers
			@server.holding_registers[2000+(id-1)*3] = 0x1060
...
# setpoint write in decimal
	...
	def set_mode(id, mode)
		...
		if (id-1)%8 == 0
			if mode == 1
				@server.input_registers[reg] = (@server.input_registers[reg] & 0xf0f0) | 0x101
				2.upto 8 do |i| set_mode (id-1)%8+i,1; end;
			elsif mode == 2 || mode == 7
				amode = mode+0x200
				@server.input_registers[reg] = (@server.input_registers[reg] & 0xf0f0) | amode
				2.upto 8 do |i| set_mode (id-1)%8+i,mode; end;
			elsif mode == 3
				@server.input_registers[reg] = (@server.input_registers[reg] & 0xf0f0) | 0x203
				2.upto 8 do |i| set_mode (id-1)%8+i,1; end;
			else
				@server.input_registers[reg] = (@server.input_registers[reg] & 0xf0f0)
				2.upto 8 do |i| set_mode (id-1)%8+i,0; end;
			end
		else
			addr = ((id-1)/8).to_i*48+2001
			if mode == 0
				@server.input_registers[reg] = (@server.input_registers[reg] & 0xf0f0) | mode
			elsif mode == 6 or mode == 1 or mode == 2
				mode = @server.input_registers[addr] & 0xf0f
				@server.input_registers[reg] = (@server.input_registers[reg] & 0xf0f0) | mode
			elsif mode == 7 and ((@server.input_registers[addr] >> 8) & 0xf) == 2
