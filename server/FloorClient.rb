...
require_relative 'IrSvm'
require_relative 'VamSvm'

class FloorClient < RoomClient
	...
	def changed?(attr)
		...
		return true if(@user != attr['passwd'])
		return false
	end
...
#					next if(point['type'] != 'Fcu')
	...
	def make_point(point_info)
		...
		when 'SPi'
			point = SPiSvm.new(point_info['pid'],point_info['dev_id'])
		when 'Ai'
			point = AiSvm.new(point_info['pid'],point_info['dev_id'])
		when 'Pi'
			point = PiSvm.new(point_info['pid'],point_info['dev_id'])
		when 'Di'
			point = DiSvm.new(point_info['pid'],point_info['dev_id'])
		when 'Dio'
			point = DioSvm.new(point_info['pid'],point_info['dev_id'])
		when 'LevelSw'
			point = LevelSwSvm.new(point_info['pid'],point_info['dev_id'])
		when 'RgbLevel'
			point = RgbLevelSvm.new(point_info['pid'],point_info['dev_id'])
		when 'KeyLock'
			point = KeyLockSvm.new(point_info['pid'],point_info['dev_id'])
		when 'Shutter'
			point = ShutterSvm.new(point_info['pid'],point_info['dev_id'])
		when 'Ir'
			point = IrSvm.new(point_info['pid'],point_info['dev_id'])
		when 'Vam'
			point = VamSvm.new(point_info['pid'],point_info['dev_id'])
		end
		if(point != nil) 
